...
const Notifications = () => {
  ...
  const handleDeleteAll = () => {
    ...
    if (
      window.confirm(
        `you have ${newArr.length} unread notifications . are you sure you want to delete `
      )
    ) {
  ...
  return (
    <div className="notifications">
      <div className="notificationheader">
        ...
        <h5
          className="notificationheaderheding"
        ...
        notify?.data.map((dt, index) => (
          <div className="notificationdata" key={index}>
            <Link to={`${dt.url}`} onClick={() => isReadNotify(dt)}>
              <div className="notificationdata-top">
                ...
                <div className="notificationdata-topsecond">
                  <h4 className="notificationdata-topsecondhead">
                    {" "}
                    {dt?.user?.fullname} {dt.text}{" "}
                  </h4>
                  <h6 className="notificationdata-topsecondheadtwo">
                    {" "}
                    {dt?.content?.slice(0, 20)}{" "}
                  </h6>
                </div>
                <img
            ...
            <div className="notificationdatabottom">
              <small className="notificationdatabottomdate">
                {moment(dt?.createdAt).fromNow()}
              </small>
              {dt.isRead ? (
                <p> o </p>
              ) : (
                <p style={{ color: "red", fontSize: "20px" }}> o </p>
              )}
            </div>
          </div>
        ))}
